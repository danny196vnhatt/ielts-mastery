...
export default function IpadDashboard() {
  ...
  const [isFolderModalOpen, setIsFolderModalOpen] = useState(false); // Modal thêm folder
  const [editingId, setEditingId] = useState<string | null>(null);
  
  // State quản lý ấn giữ để xóa
  ...
  const [vocabList, setVocabList] = useState(initialData);
  const [folders, setFolders] = useState([
    { id: 'f1', label: 'Writing Task 2', topics: ['Environment', 'Education', 'Society', 'Health'], color: 'bg-orange-500' },
    { id: 'f2', label: 'Speaking Part 1', topics: ['Speaking'], color: 'bg-blue-500' }
  ]);
  ...
  useEffect(() => {
    ...
    // Đóng nút xóa khi click ra ngoài
  ...
  const handleSaveWord = () => {
    const formattedWord = {
      ...
      status: editingId ? vocabList.find(v => v.id === editingId)?.status : 'new',
    ...
    const newList = editingId ? vocabList.map(v => v.id === editingId ? formattedWord : v) : [...vocabList, formattedWord];
    ...
    <div className="flex h-screen bg-[#F8F9FA] text-slate-900 overflow-hidden font-sans select-none">
      ...
      <aside className={`${isSidebarOpen ? 'w-72' : 'w-0'} transition-all duration-300 bg-white border-r border-gray-100 flex flex-col overflow-hidden`}>
        ...
        <nav className="flex-1 px-4 space-y-1 overflow-y-auto">
          ...
          {folders.map(folder => (
            <div 
              ...
              onMouseDown={() => handleTouchStart(folder.id, 'folder')}
              onMouseUp={handleTouchEnd}
              onTouchStart={() => handleTouchStart(folder.id, 'folder')}
              onTouchEnd={handleTouchEnd}
            >
              <FolderItem 
                label={folder.label} 
                count={vocabList.filter(v => folder.topics.includes(v.topic)).length} 
                color={folder.color} 
                active={activeFolderId === folder.id}
                onClick={() => { if(!showDeleteFolderId) { setActiveFolderId(folder.id); setViewMode('grid'); }}}
              />
              {showDeleteFolderId === folder.id && (
                <button 
                  onClick={(e) => { e.stopPropagation(); deleteFolder(folder.id); }}
                  className="absolute right-0 inset-y-0 w-12 bg-red-500 text-white flex items-center justify-center rounded-xl animate-in slide-in-from-right-full"
                >
                  <Trash2 size={16}/>
                </button>
              )}
        ...
        <div className="p-4 border-t space-y-2">
          <button onClick={() => {setEditingId(null); setIsModalOpen(true);}} className="w-full flex items-center justify-center gap-2 py-4 bg-blue-600 text-white rounded-2xl font-black shadow-lg hover:bg-blue-700">
      ...
      <main className="flex-1 flex flex-col relative bg-[#F2F4F7]">
        <button onClick={() => setSidebarOpen(!isSidebarOpen)} className="absolute top-6 left-4 z-50 p-2 bg-white shadow-sm rounded-xl border border-gray-100">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {activeFolder.topics.map(topic => (
                <TopicCard 
                  ...
                  total={vocabList.filter(v => v.topic === topic).length || 1} 
            ...
            <div className="space-y-3">
              {filteredVocab.map(word => (
                <div 
                  ...
                  onMouseDown={() => handleTouchStart(word.id, 'word')}
                  onMouseUp={handleTouchEnd}
                  onTouchStart={() => handleTouchStart(word.id, 'word')}
                  onTouchEnd={handleTouchEnd}
                >
                  <div className="bg-white p-4 flex justify-between items-center shadow-sm">
                    <div>
                      <span className="font-black text-lg">{word.word}</span>
                      <span className="ml-3 text-xs font-bold text-slate-400 uppercase">{word.type}</span>
                    </div>
                    <div className="flex gap-2">
                      <button onClick={() => openEditModal(word)} className="p-2 hover:bg-slate-100 rounded-lg text-blue-600"><Edit2 size={18}/></button>
                    </div>
                  </div>
                  {showDeleteWordId === word.id && (
                    <button 
                      onClick={(e) => { e.stopPropagation(); deleteWord(word.id); }}
                      className="absolute right-0 inset-y-0 w-16 bg-red-500 text-white font-black flex items-center justify-center animate-in slide-in-from-right-full"
                    >
                      XÓA
                    </button>
                  )}
            ...
            {learningQueue.length > 0 ? (
              <>
                ...
                <div className="flex gap-4 mt-10">
                  ...
                  }} className="px-8 py-4 bg-green-500 text-white rounded-2xl font-black shadow-lg flex items-center gap-3"><CheckCircle2 size={24}/> ĐÃ THUỘC</button>
                  <button onClick={() => {
                    const first = learningQueue.shift();
                    if(first) learningQueue.push(first);
                    setVocabList([...vocabList]);
                  }} className="px-8 py-4 bg-white text-slate-600 border rounded-2xl font-black flex items-center gap-3">TIẾP THEO <ArrowRight size={24}/></button>
                </div>
              </>
            ) : (
      ...
      {isFolderModalOpen && (
        <div className="fixed inset-0 z-[110] bg-black/40 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-md rounded-[2.5rem] p-8 shadow-2xl space-y-4">
            ...
            <Input label="Các Topics (cách nhau dấu phẩy)" value={folderFormData.topics} onChange={(v:any) => setFolderFormData({...folderFormData, topics: v})} placeholder="Map, Process, Chart" />
          ...
          <div className="bg-white w-full max-w-2xl rounded-[2.5rem] shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
            ...
            <div className="p-8 overflow-y-auto space-y-4">
              ...
              <div className="grid grid-cols-3 gap-4">
                ...
                <Select label="Topic" options={['Education', 'Environment', 'Health', 'Society', 'Speaking']} value={formData.topic} onChange={(v:any) => setFormData({...formData, topic: v})} />
              </div>
      ...
      <aside className={`${showMotivation ? 'w-80' : 'w-0'} transition-all duration-300 bg-white border-l border-gray-100 p-6 overflow-hidden`}>
        <div className="space-y-8">
          <div className="bg-gradient-to-br from-orange-400 to-red-500 p-6 rounded-[2.5rem] text-white shadow-xl">
            ...
            <p className="font-bold uppercase text-[10px] tracking-widest">Streak liên tục</p>
          </div>
...
function NavItem({ icon, label, active, onClick }: any) {
  return (
    <div onClick={onClick} className={`flex items-center gap-4 p-4 rounded-2xl cursor-pointer transition-all ${active ? 'bg-blue-600 text-white shadow-lg shadow-blue-200' : 'hover:bg-slate-50 text-slate-500'}`}>
...
function TopicCard({ title, progress, total, onStudy }: any) {
  ...
  return (
    <div className="bg-white p-8 rounded-[3rem] shadow-sm border border-white">
      <h3 className="text-2xl font-black mb-6 italic uppercase text-[14px] tracking-widest text-slate-400">Writing: {title}</h3>
      ...
      <button onClick={onStudy} className="w-full py-3 bg-slate-50 hover:bg-blue-50 text-blue-600 rounded-xl font-bold transition-colors">BẮT ĐẦU HỌC</button>
    </div>
  )
}
